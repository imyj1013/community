...
async def create_comment(request: Request):
    ...
    try:
        ...
        comment = create_comment(post_id, user_id, content, datetime.now(timezone.utc).isoformat())
        post = post_model.update_comments_count(post, 1)
...
async def update_comment(comment_id: int, request: Request):
    ...
    try:
        ...
        comment = comment_model.update_comment(comment, content)
